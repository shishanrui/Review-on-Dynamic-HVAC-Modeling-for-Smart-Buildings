...
def SSSS(topic, sub_keyword_list, year_from, year_to, citation_threshold, number_of_searches_per_key_word_per_year = 10, sleep_interval = 360):
    ...
    :param sub_keyword_list: list of strings; defines the sub-keyword list of SSSS, example: [['energy','gas'],['prediction', 'forecasting'], ['in buildings', 'HVAC']]
    ...
    for key_words in key_words_list:
        ...
        for nth_paper in range(number_of_searches_per_key_word_per_year):
            # revised-- if nth_paper >= len(articles):
            #              break
            # 2025/05/07: articles is coming back shorter than the loop expects—sometimes
            # even empty—so the code tries to fetch articles[nth_paper] when
            # that element doesn’t exist and Python raises IndexError
            if nth_paper >= len(articles):
                break
            ...
            if (title_nth not in summary_df.title.tolist()) & (num_citations_nth >= citation_threshold):
                ...
                summary_df.to_csv('../results/topics/{}/summary.csv'.format(topic), index=False, encoding='utf-8-sig')
